...
<head>
    ...
    <title>Nouvelle Ferme | Tableau de Bord</title>
...
<body>
    <div class="form-container">
        <div class="form-header">
            <h1>Nouvelle Ferme</h1>
            <p>Remplissez les informations pour créer une nouvelle ferme</p>
        </div>

        <form method="POST" action="{{ route('fermes.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select id="orientation_id" name="orientation_id" class="form-input @error('orientation_id') input-error @enderror" required>
                        ...
                        @foreach($orientations as $orientation)
                            <option value="{{ $orientation->id }}" {{ old('orientation_id') == $orientation->id ? 'selected' : '' }}>{{ $orientation->nom }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group full-width">
                    ...
                    <select id="infrastructures" name="infrastructures[]" class="form-input @error('infrastructures') input-error @enderror" multiple>
                        <option value="">Sélectionnez une ou plusieurs infrastructures</option>
                        @foreach($infrastructures as $infrastructure)
                            <option value="{{ $infrastructure->id }}" {{ in_array($infrastructure->id, old('infrastructures', [])) ? 'selected' : '' }}>{{ $infrastructure->nom }}</option>
                        @endforeach
                    </select>
            ...
            <button type="submit" class="btn-submit">
                <i class="fas fa-tractor"></i> Créer la ferme
            </button>
        </form>
    </div>
    ...
    <script>
        $(document).ready(function() {
            // Initialiser Select2 pour les infrastructures
            $('#infrastructures').select2({
                placeholder: "Sélectionnez une ou plusieurs infrastructures",
                allowClear: true,
                width: '100%'
            });
            ...
            // Déclencher le changement initial si une ville est déjà sélectionnée (par exemple, après une erreur de validation)
            if ($('#ville_id').val()) {
